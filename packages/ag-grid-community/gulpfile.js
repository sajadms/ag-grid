...
const tscSrcTask = () => {
    ...
    const tsResult = gulp.src('src/ts/**/*.ts').pipe(tsProject());

    return merge([
        tsResult.dts.pipe(header(dtsHeaderTemplate, { pkg: pkg })).pipe(gulp.dest('dist/lib')),
        tsResult.js.pipe(header(headerTemplate, { pkg: pkg })).pipe(gulp.dest('dist/lib'))
    ]);
};
...
const tscMainTask = () => {
    ...
    const tsResult = gulp.src('./src/ts/main.ts').pipe(tsProject());
...
const tscModulesTask = () => {
    ...
    const tsResult = gulp.src(['./src/ts/modules/**/*Module.ts']).pipe(tsProject());
...
const cleanMain = () => {
    return gulp.src(['./main.d.ts', 'main.js'], { read: false, allowEmpty: true }).pipe(clean());
};
const cleanModules = () => {
    return gulp.src(['./*Module.d.ts', './*Module.js'], { read: false, allowEmpty: true }).pipe(clean());
};
...
const tscSrcEs6Task = () => {
    ...
    return merge([
        tsResult.dts.pipe(header(dtsHeaderTemplate, { pkg: pkg })).pipe(gulp.dest('dist/es6')),
        tsResult.js.pipe(header(headerTemplate, { pkg: pkg })).pipe(gulp.dest('dist/es6'))
    ]);
};
...
const scssTask = () => {
    ...
    return gulp
        .src(['src/styles/**/*.scss', '!src/styles/**/_*.scss', '!**/node_modules/**/*.scss'])
        .pipe(named())
        .pipe(
            webpackStream({
                mode: 'none',
...
gulp.task(
    'webpack-all',
    series('clean', 'tsc-scss-no-clean', parallel('webpack-no-clean', 'webpack-minify-no-clean', 'webpack-noStyle-no-clean', 'webpack-minify-noStyle-no-clean'))
);
